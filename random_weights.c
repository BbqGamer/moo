...
void read_predicted_returns(double *predicted_returns, const char *filename) {
    ...
    for (int i = 0; i < NUM_ASSETS; i++) {
        ...
        // printf("predicted_returns[%d] = %f\n", i, predicted_returns[i]); // Debug statement
    }
...
void read_cov_matrix(double cov_matrix[NUM_ASSETS][NUM_ASSETS], const char *filename) {
    FILE *file = fopen(filename, "r");
    if (file == NULL) {
        perror("Error opening file");
        exit(1);
    ...
    while (fgets(line, sizeof(line), file)) {
        ...
        for (int col = 0; col < NUM_ASSETS; col++) {
            ...
            // printf("cov_matrix[%d][%d] = %f\n", row, col, cov_matrix[row][col]); // Debug statement
            token = strtok(NULL, ",");
        }
        if (token != NULL) {
...
int main() {
    ...
    if (random_profit == NULL || random_risk == NULL) {
